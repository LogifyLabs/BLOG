...
이는 특정 [[클래스 (Classes)]]의 개체(Individual)가 특정 [[Object Properties]])를 가질 경우, 그 대상(object)이 **"오직(only)" 명시된 클래스(들)의 구성원이어야만 한다**고 제한하는 [[공리 (Axioms)]]입니다.
...
- **예시:** 
    ...
    - **해석**: "모든 `BicCristal` 펜은, `hasComponent`를 _가진다면_, 그 대상은 _오직_ `BallPoint`, `Cap`, `InkReservoir`, `Body` 클래스 중 하나여야만 한다."
...
**연결 노트:** [[OWL 제한 (Restrictions)]], [[Existential Restriction (some)]], [[공식화 (Formalization)]], [[Object Properties]], [[OWL 추론(Reasoning)]], [[클래스 (Classes)]], [[전칭 제한 (Universal Restriction)]], [[Practical Knowledge Modelling-Ontology Development 101]]
